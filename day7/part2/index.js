...
const solve = async () => {
  ...
  do{
    ...
    let availableWorkers = workers.filter(wrkr => !wrkr.currentlyOn)
    for(let i = 0; i<availableWorkers.length; i++){
      let currentlyBeingWorkedOn = workers.filter(wrkr => wrkr.currentlyOn).map(wrkr => wrkr.currentlyOn.id);
      let nextLocation = getNextLocations(visited, currentlyBeingWorkedOn, allLetters, input);
